...
<?php
    ...
    $output_box .= "\n";
        ...
        foreach($wordpress_database_boxes as $wordpress_info_box) {
            ...
            if( ($display_start <= $num) && ($display_end >= $num) ) { 
                ...
                if( !empty($wordpress_info_box->url_link) ) {
                    $box_url_open = "<a" . $open_new_tab . " href='" . esc_url($wordpress_info_box->url_link) . "'>";
                    $box_url_close = "</a>";
                } // if( !empty($wordpress_info_box->url_link) ) {
                ...
                $output_box .= "<div class='yydev_box " . esc_attr($box_type_class) . " " . esc_attr($image_class) . " " . esc_attr($yydev_box_id_class) . "' " . $box_style_height_width . ">";
                ...
                if( !empty($wordpress_info_box->image_path) ) {
                    ...
                    $output_box .= "<iframe src='" . esc_url($wordpress_info_box->image_path) . "' width='" . esc_attr($wordpress_info_box->box_width) . "' height='" . esc_attr($wordpress_info_box->box_height) . "'  allowfullscreen></iframe>";
        ...
        // ================================================
        $output_box_pages = yydev_portfolio_create_pages($items_number, $itesm_per_page, $page_path, $current_page_number, "bottom-pagination");
        $output_box .= __($output_box_pages, 'yydevelopment-portfolio-boxes');
