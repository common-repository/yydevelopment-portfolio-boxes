...
<?php
    ...
    if( !empty($main_box_info->container_width) ) {
        $container_width_style =  " style='max-width:" . esc_attr($main_box_info->container_width) . ";'";
    ...
    $output_box .= "\n";
        $output_box .= "<div class='yydev_portfolio " . esc_attr($main_box_info->slug) . " yydev_portfolio_" . esc_attr($main_box_info->id) . esc_attr($output_box_class) . "'" . $container_width_style . ">";
        ...
        foreach($wordpress_database_boxes as $wordpress_info_box) {
            ...
            if( ($display_start <= $num) && ($display_end >= $num) ) { 
                ...
                $nofollow_link = "";
                if($wordpress_info_box->nofollow_link  == 1) {
                    $nofollow_link = " rel='nofollow' ";
                } // if($wordpress_info_box->nofollow_link  == 1) {

                if( !empty($wordpress_info_box->url_link) ) {
                    $box_url_open = "<a" . $open_new_tab . " href='" . esc_url($wordpress_info_box->url_link) . "'" . $nofollow_link . ">";
                    $box_url_close = "</a>";
                } // if( !empty($wordpress_info_box->url_link) ) {
                ...
                if( !empty($main_box_info->box_height) || !empty($wordpress_info_box->box_height) || !empty($main_box_info->box_width) || !empty($wordpress_info_box->box_width) ) {
                    ...
                    if( !empty($main_box_info->box_height) || !empty($wordpress_info_box->box_height) ) {
                        ...
                        $box_style_height_width .= "min-height:". esc_attr($box_height_value) . ";";
                    } // if( !empty($main_box_info->box_height) || !empty($wordpress_info_box->box_height) ) {
                    ...
                    if( !empty($main_box_info->box_width) || !empty($wordpress_info_box->box_width) ) {
                        ...
                        $box_style_height_width .= "max-width:". esc_attr($box_width_value) . ";";
                    } // if( !empty($main_box_info->box_width) || !empty($wordpress_info_box->box_width) ) {
                ...
                if( !empty($main_box_info->box_type) ) {
                    ...
                    $output_box .= "<div class='yydev_box " . esc_attr($box_type_class) . esc_attr($image_class) . "' " . esc_attr($box_style_height_width) . ">";
                ...
                if(strstr($image_path_link, ".swf")) {
                    ...
                    $output_box .= "<embed src='" . esc_url($image_path_link) . "' width='" . esc_attr($image_path_link_size[0]) . "' height='" . esc_attr($image_path_link_size[1]) . "' />";
                    ...
                    if( !empty($wordpress_info_box->image_path) ) {
                        $get_img_box_link = "<img src='" . $wordpress_info_box->image_path . "' alt='" . yydev_portfolio_boxes_html_output($wordpress_info_box->image_alt) . "' />";
        ...
        // ================================================
        $output_box .= yydev_portfolio_create_pages($items_number, $itesm_per_page, $page_path, $current_page_number, "bottom-pagination");
        ...
        if( is_user_logged_in() ) {
            $get_plugin_path_page = get_bloginfo('url') . "/wp-admin/admin.php?page=yydev_portfolio&view=box&id=" .  intval($box_id);
            $path_plugin_link = "<a class='portfolio-edit-works-button' href='" . esc_url($get_plugin_path_page) . "'>" . __('Click Here To Edit the Works', 'yydevelopment-portfolio-boxes') . "</a>";
